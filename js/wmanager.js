	...
	window.wmanager = {
		...
		open: function(options) {
			...
			if( wmanager.globals.taskbar !== false) {
				...
				.data('wm_wid', op.wid)
				.click( function() {
					if( !empty(op.open && $.isFunction(op.open)) ) {
						op.open();
					} else {
        ...
        windowHidden: function(wid) {
            return isset(wmanager.windows[wid]) && $('.'+ wid).is(':hidden');
        },
		utils: {
